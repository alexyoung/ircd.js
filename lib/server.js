...
winston.configure({
  ...
  transports: [
    ...
    new winston.transports.File({ filename: 'ircd.log', level: 'verbose' })
  ]
});
...
Server.prototype = {
  ...
  /*
    :nick!user@host AWAY :bye bye
    {
      args: ["nick!user@host", "bye bye"],
      command: "AWAY"
    },

    :foo!foo@example.com PRIVMSG #channel :I like turtles.
    {
      args: ["foo!foo@example.com", "#channel", "I like turtles."]
      command: "PRIVMSG"
    }
  */
  parse: function(data) {
  ...
  // TODO how does this work? only if serverPassword is set, commands stored before authenticating?
  ...
  startTimeoutHandler: function() {
    ...
    this.timeoutHandler = setInterval(function() {
      self.users.forEach(function(user) {
        if (user.hasTimedOut()) {
          winston.verbose('User timed out:', user.mask);
  ...
  start: function(callback) {
    ...
    function handleStream(stream) {
      try {
        ...
        winston.info(client.object.remoteAddress + ' joined')
  ...
  close: function(callback) {
    ...
    if (user) {
      winston.info(user.remoteAddress + ' left')
  ...
  data: function(client, line) {
    ...
    winston.verbose({ client: client.id + '@' + this.name, data: line, message: 'incoming'});
