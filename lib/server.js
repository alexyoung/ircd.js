...
Server.prototype = {
  ...
  start: function(callback) {
    ...
    this.config.listen.forEach(function(listener) {
      if(listener && listener.port && typeof(listener.port) === 'number') {
          var secure = false;
          var newServer = null;

          if(listener.secure !== undefined && typeof(listener.secure) === 'boolean')
            secure = listener.secure;

          if(secure && listener.key && listener.cert) {
            var key, cert;
            try {
              key = fs.readFileSync(listener.key);
              cert = fs.readFileSync(listener.cert);
            } catch (exception) {
              winston.error('Fatal error:', exception);
              return;
            }

            newServer = tls.createServer({key: key, cert: cert}, handleStream);
          }
          else {
            newServer = net.createServer(handleStream);
          }

          assert.ok(callback === undefined || typeof callback == 'function');
          socketListeners.push(newServer);
          newServer.listen(listener.port, callback);
      }
    });

    this.listeners = socketListeners;
